...
#let split-box(
  ...
  common-box(body: body1,boddy-size: 80%, ..args)
}
...
#let bottom-box(body, text-relative-width: 90%, logo: none, ..args) = {
  let body = [
    #set align(top+left)
    #if logo==none {
      box(width: 100%, body)
...
#let bottom-box2(body, text-relative-width: 90%, ..args) = {
  let body = [
    #set align(top+left)
    #text(
      stack(dir: ltr,
...
#let middle-box(left-body, right-body, left-width: 80%, right-width: 20%, ..args) = {
  // Create the horizontal layout
  let left-body =stack(dir: ltr,                 // left-to-right stacking
    box(width: left-width, left-body),
  )
  let right-body =stack(dir: ltr,                 // left-to-right stacking
    box(width: right-width, right-body),
  )
  
  // Use same visual style as other boxes
  let r1 = common-box(heading: body, bottom-box: false, ..args)
  let r2 = common-box(heading: body, bottom-box: false, ..args)
  // Align in the vertical middle (optional)
  align(center, r1,r2)
}
